...
//    @State var friends = [User]()
    ...
    var body: some View {
        List(){
            ForEach(loginManager.friends, id: \.self){ friend in
                RowViewFriend(name: friend.displayName!)
            }
            
            if loginManager.friends.isEmpty {
                Text("You have no friends :(")
            }
        }
        .navigationTitle("Friends")
        .onAppear(){
            loginManager.updateFriends(friends: loginManager.friends) { success in
                
                print("updatefriends? \(success)")
            }
            
            loginManager.getFriends(){ success in }
